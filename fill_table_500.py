    ...
    with open(file_path, 'r') as f:
        try:
            ...
            result = subprocess.run(
                ['python3', 'get_n.py'],  # Run the script (can be replaced with your script)
            ...
            print(".", end="")
            results_nodes[file_name] = result.stdout.strip()  # .strip() to remove any extra newlines
...
for file_path in glob.glob('from_thore/data/*.txt'):
    ...
    if int(results_nodes[file_name]) >= 500:
        with open(file_path, 'r') as f:
            try:
                # Run the subprocess and capture the output
                result = subprocess.run(
                    ['python3', 'none.py'],  # Run the script (can be replaced with your script)
                    stdin=f,  # Provide input via stdin
                    capture_output=True,  # Capture stdout and stderr
                    text=True  # Make sure the output is captured as text
                )
                ...
                # Store the filename and its corresponding output in the dictionary
                print(".", end="")
                results_none[file_name] = result.stdout.strip()  # .strip() to remove any extra newlines
    ...
    if int(results_nodes[file_name]) >= 500:
        with open(file_path, 'r') as f:
            try:
                # Run the subprocess and capture the output
                result = subprocess.run(
                    ['python3', 'few.py'],  # Run the script (can be replaced with your script)
                    stdin=f,  # Provide input via stdin
                    capture_output=True,  # Capture stdout and stderr
                    text=True  # Make sure the output is captured as text
                )
                ...
                # Store the filename and its corresponding output in the dictionary
                print(".", end="")
                results_few[file_name] = result.stdout.strip()  # .strip() to remove any extra newlines
    ...
    if int(results_nodes[file_name]) >= 500:
        with open(file_path, 'r') as f:
            try:
                # Run the subprocess and capture the output
                result = subprocess.run(
                    ['python3', 'alternate.py'],  # Run the script (can be replaced with your script)
                    stdin=f,  # Provide input via stdin
                    capture_output=True,  # Capture stdout and stderr
                    text=True  # Make sure the output is captured as text
                )
                ...
                # Store the filename and its corresponding output in the dictionary
                print(".", end="")
                results_alternate[file_name] = result.stdout.strip()  # .strip() to remove any extra newlines
    ...
    if int(results_nodes[file_name]) >= 500:
        with open(file_path, 'r') as f:
            try:
                # Run the subprocess and capture the output
                result = subprocess.run(
                    ['python3', 'many.py'],  # Run the script (can be replaced with your script)
                    stdin=f,  # Provide input via stdin
                    capture_output=True,  # Capture stdout and stderr
                    text=True  # Make sure the output is captured as text
                )
                ...
                # Store the filename and its corresponding output in the dictionary
                print(".", end="")
                results_many[file_name] = result.stdout.strip()  # .strip() to remove any extra newlines
...
with open('results_500.txt', 'w') as f:
    ...
    for file_name in instances:
        if int(results_nodes[file_name]) >= 500:
            ...
            nodes = results_nodes.get(file_name, "Timeout" if results_nodes.get(file_name) is None else "-1")
            A = results_alternate.get(file_name, "Timeout" if results_alternate.get(file_name) is None else "-1")
